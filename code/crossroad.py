...
try:
    ...
    spawn_point0 = Transform(Location(x=-43, y=41, z=3), Rotation(pitch=0, yaw=270, roll=0))
    spawn_point1 = Transform(Location(x=-60, y=25, z=3), Rotation(pitch=0, yaw=0, roll=0))
    spawn_point2 = Transform(Location(x=-30, y=13, z=3), Rotation(pitch=0, yaw=180, roll=0))
    ...
    car1.apply_control(carla.VehicleControl(throttle=0.6, steer=0))
    time.sleep(2)
    
    node_num = 10 #每条路径规划的点数
    localtime = 0
    
    rate_car = 0.5 #标签概率，对car2采用
    rate_man = 0.5
    rate_bike = 0
    ...
    while True:
        ...
        t = 0.1 #不同节点间的时间
        #意图：本文件的对应工况是交叉路口转弯，这是由行为决策做出的判断
        if mycary >= 37 and mycarx >= -51:
            targetx = mycarx
            targety = mycary - 12
        elif mycary < 37 and mycary >= 25 and mycarx >= -51:
            targetx = -51 + (8**2 - (mycary-25)**2)**(1/2)
            targety = mycary - (12**2 - (mycarx - targetx)**2)**(1/2)
        elif mycarx >= -51 and mycary < 25:
            targety = 17
            targetx = mycarx - (12**2 - (mycary - targety)**2)**(1/2)
        elif mycarx < -51 and mycary < 25:
            targetx = -12 + mycarx
            targety = 17
        else:
            print('mycar out of crossroad range')
            targetx = -51
            targety = 17
        ...
        for ratio in ratio_list:
            ...
            for i in range(1, node_num): #已有0，规划1~num-1，共num个
                ...
                fy = f1y + rate_car * f2y + rate_man * fmy + fty
    ...
    def basis(i):
        ...
        print('sum: ' + str((p1 + p2 + pt + pm)[25][25]))
        Z = np.minimum(p1 + p2 + pt + pm, 1000000)
        ...
        x = np.linspace(-80, -20, num=50)
        y = np.linspace(-35, -5, num=50)
        [X, Y] = np.meshgrid(x, y)
        p1 = field.CarPotentialField(car1m, car1_x, car1_y, car1_vx, car1_vy).get_potential(X, Y, 0, 0)
        p2 = field.CarPotentialField(car2m, car2_x, car2_y, car2_vx, car2_vy).get_potential(X, Y, 0, 0) * rate_car
        pt = field.TargetField(targetx, -targety, 1).get_potential(X, Y)
