...
def basis(i):
    ...
    x0 = 2 - i * 0.01
    y0 = 10 - x0**2 / 2
    ax.scatter(x0, y0, color='black', linewidths=10) #red
    y2 = 9.75 - 0.875 * x0**2 / 2
    ax.scatter(x0, y2, color='black', linewidths=10)
    
    #画红色轨迹
    x = np.linspace(x0, 2, num=100)
    ax.plot(x, 10 - x**2 / 2, color='black') #red
    
    ax.plot(x, 9.75 - 0.875 * x**2 / 2, color='black')
    ...
    for point in point_list:
        ax.scatter(point[0], point[1], color='black', linewidths=10) #red
    for point in point_list2:
        ax.scatter(point[0], point[1], color='black', linewidths=10)

animator = ani.FuncAnimation(fig, basis, interval=30)
plt.show()
