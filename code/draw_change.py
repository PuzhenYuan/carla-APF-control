...
try:
    ...
    while True:
        ...
        car1.apply_control(carla.VehicleControl(throttle=0, steer=0))  #throttle=0~0.3
        car2.apply_control(carla.VehicleControl(throttle=0.2, steer=0))  #throttle>=0
        
        print('---------------')
        print('steer: ' + str(steer))
        print('theta: ' + str(target_theta - path[0].theta))
        
        # if crash:
        #     time.sleep(4)
        
        localtime = localtime + 1
        if localtime > 1300: #退出机制
    ...
    def basis(i):
        ...
        ax1 = plt.axes(rect1)
        ax1.axis([-4+xb, 4+xb, -4+yb, 4+yb])
        ax1.set_xlabel('x-position')
        ax1.set_ylabel('y-position')
        ax1.set_title('local map')
        
        x = np.linspace(-4+xb, 4+xb, num=100)
        y = np.linspace(-4+yb, 4+yb, num=100)
        [X, Y] = np.meshgrid(x, y)
