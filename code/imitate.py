...
try:
    ...
    man_bp = blue_print_library.find('walker.pedestrian.0001')
    
    weather = world.get_weather()
    weather.sun_altitude_angle = -60
    weather.fog_density = 100
    weather.fog_distance = 60
    world.set_weather(weather)
    
    light_manager = world.get_lightmanager()
    light_group = light_manager.get_all_lights()
    light_manager.turn_off(light_group)
    
    spawn_point0 = Transform(Location(x=65, y=25, z=1), Rotation(pitch=0, yaw=180, roll=0))
    spawn_point1 = Transform(Location(x=50, y=28.5, z=1), Rotation(pitch=0, yaw=180, roll=0))
    spawn_point2 = Transform(Location(x=30, y=25.8, z=1), Rotation(pitch=0, yaw=180, roll=0))
    
    mycar = world.spawn_actor(car_bp, spawn_point0)
    car1 = world.spawn_actor(car_bp, spawn_point1)
    car2 = world.spawn_actor(man_bp, spawn_point2)
    ...
    # current_lights != carla.VehicleLightState.LowBeam
    mycar.set_light_state(current_lights)
    car1.set_light_state(current_lights)
    ...
    transform = mycar.get_transform()
    spectator.set_transform(carla.Transform(transform.location+carla.Location(x=1,z=1.45),carla.Rotation(pitch=-10,yaw=180)))
    ...
    while True:
        ...
        time_b = time.time()
        transform = mycar.get_transform()
        spectator.set_transform(carla.Transform(transform.location+carla.Location(x=1,z=2),carla.Rotation(pitch=-10,yaw=180)))
        ...
        car1.apply_control(carla.VehicleControl(throttle=0.35, steer=0))  #throttle=0~0.3
        car2.apply_control(carla.WalkerControl(direction=carla.Vector3D(0, -1, 0), speed=0.2, jump=False))
        
        # if crash:
        #     time.sleep(4)
        
        localtime = localtime + 1
        if localtime > 3000: #退出机制
